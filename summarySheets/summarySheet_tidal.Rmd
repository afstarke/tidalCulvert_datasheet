...
css: summary.css
params:
  crossingCode: xy4080650672832426
---
...
tidalPrioritization <- read_rds(here::here("data", "/LIculvertPrioritizations.rds")) # Prioritization data
...
photoLookup <- read_csv(here::here("summarySheets/tidalphotos/match_table_021820.csv")) # photos will likely need to be local to ensure they render in htmls.
photoFolder <- "tidalphotos/"
...
coords <- tidal_doc_data %>% st_drop_geometry() %>% 
...
tidal_table_data <- tidal_doc_data %>% 
  mutate(Coordinates = paste(round(latitude, digits = 5), "N ", round(longitude, digits = 5), "W", sep = ""),
         type_material = paste(fw_doc_data$Culvert_Material, fw_doc_data$CrosType, sep = " ")) %>% 
  ...
  dplyr::right_join(tsdn) %>% # join in the tidal data that's of interest from 

...
profilePlot_default <-
  read_rds(here::here("data", "/LIculvertAssessments.rds")) %>%
  filter(crossingID == 113) %>% pull(profilePlots) 
...
```{r profilePlots, fig.height=6, fig.width=7, message=FALSE, warning=FALSE, paged.print=FALSE}


pplot <-
  profilePlot[[1]]$result
...
if (!is.null(pplot)) {
  ...
  theme(
    ...
    # panel.grid.minor = element_blank(),
    # plot.background = element_blank()
  ) + labs(title = "Mock crossing",
           subtitle = glue::glue("Crossing #: {ccode}"))
}

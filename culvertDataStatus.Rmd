...
LIculvertData_location %>% select(crossingID, starts_with("da_"), -ends_with("Comts")) %>% 
  st_drop_geometry() %>% 
  plot_missing(ggtheme = theme_ipsum())
```
...
LIculvertDataStatus_location %>% 
  select(crossingID, starts_with("da_"), 
         -starts_with("da_NW"),
         ends_with("Comts")) %>% mutate(propNa = rowMeans(is.na(.), na.rm = T)) %>% 
...
LIculvertsAssessments %>% select(crossingID, heights) %>%  
  unnest() %>% mutate(hasHt = !is.na(adjustedHt) | !is.na(Height)) -> hwi_qunat

hwi_qunat %>% filter(hasHt == TRUE) %>% select(crossingID, Feature, Position)  %>% rpivotTable(rows = "crossingID", cols = c("Feature", "Position"), rendererName = "Heatmap", aggregatorName = "Count")

...
source(here::here("functions", "prioritization_functions.R"))
# libraries used elsewhere and stored here for convienence
...
library(kableExtra)
library(DT)
theme_set(theme_tnc_base() + theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"), text = element_text(size = 11)))
...
LIculvertsAssessments <- read_rds(here("data","LIculvertsAssessments.rds")) # tidal assessment workbooks, with varying degrees of completness. This is the bulk of the data collected, but does not include complete desktop assessments. Those data are recorded and managed in the AGOL-hosted feature _desktopData_. 
LIculvertData_location <- read_rds(here("data", "LIculvertData_location.rds")) # tidal assessment data joined to locations of crossings identified in assessments. 
...
LIculvertAssessmentData %>% write_rds(here("data", "LIculvertAssessmentData.rds")) # Data are updated each run.
```
...
_DECIDED ON_ likely not to included since it measures 
...
dtr <- LIculvertPrioritization %>% rowwise() %>% 
  ...
  mutate(Score_dotr = round(degtidalrestr(tidal_rng_score = submetric_TidalRange, 
         ...
         Score_dotr_unround = degtidalrestr(tidal_rng_score = submetric_TidalRange, 
                                         crossing_ratio_score = submetric_crossingRatio,
                                         erosion_class_score = submetric_erosionClass),
         Score_dotr_binned = cut(x = Score_dotr_unround, breaks = c(-Inf, 1.4999, 2.4999, 3.4999, 4.4999, Inf), labels = c(1:5), ordered_result = F)) %>% 
...
LIculvertPrioritization <- LIculvertPrioritization %>%
  left_join({dtr %>% select(crossingID, Score_dotr_binned) %>% rename(Score_dotr = Score_dotr_binned)}, by = "crossingID") # TODO
...
LIculvertPrioritization <- LIculvertPrioritization %>% rowwise() %>% 
  mutate(Total_Eco = benefit_score(sum(Score_MarshComplex, Score_marshAc, Score_dotr, Score_vegetation, na.rm = F))) %>% # NOTE: na.rm = T to drop NA before summing 
  ungroup()
...
TODO: make these formuals 2 unique ones for each metric.
hwi-loc ratio:
$$ scaleAdjust^{high water} = minimum(hwi^{stain} | \ road \ center \ height )$$
$$ scaleAdjust^{lack \ of \ clearance} = minimum(hwi^{stain} | \ ceiling \ of \ structure )$$
$$ highwater \ clearance \ risk \ ratio = scaleAdjust + (hwi^{wrack} |  hwi^{wrack}) / scaleAdjust + Road \ Height$$
...
highwater <- LIculvertAssessmentData %>%
  ...
  mutate(
    ...
    highwater_dif_US = `Road Surface US` - `HWI Wrack US` ,
    # scored 
...
#   select(crossingID, c(starts_with("highwater"), starts_with("clearance"))) %>% pivot_longer(cols = -crossingID) %>% tidyr::replace_na(replace = list('value' = -10)) %>% 
#   ggplot(aes(y = as.character(crossingID), x = value, shape = name, color = name)) + geom_point() + scale_color_tnc("oceanhighlight") + theme(panel.grid.major.y = element_line()) -> plot
# ggplotly(plot, height = 1800)
# 
#     
# highwater %>% st_drop_geometry %>% 
#   select(crossingID, starts_with("score")) %>% pivot_longer(cols = -crossingID) %>% tidyr::replace_na(replace = list('value' = 0)) %>% 
#   ggplot(aes(y = as.character(crossingID), x = value, shape = name, color = name)) + geom_point() + scale_color_tnc("oceanhighlight") + theme(panel.grid.major.y = element_line()) -> plot2
# ggplotly(plot2, height = 1800)
...
highwater %>% 
  ...
  geom_text(stat = "count", aes(label = ..count..), position = position_stack(vjust = 0.5)) +
  scale_fill_tnc("gradient1") + 
  labs(title = "Risk of high water \nLack of clearance", subtitle = "following NH bins; worst-score wins", fill = "proposed scoring")
...
write_rds(LIculvertPrioritization, path = here("data", "LIculvertPrioritizations.rds"))

...
# To knit both HTML and Word doc use render("tidalCrossing_Prioritizations.Rmd", format = "all") in console.
...
This document serves two purposes related to the prioritization of tidal crossings assessed under the Long Island Road-Stream Crossings Prioritization project.
It provides an outline of the workflow and methods used for the calculation of the prioritization metrics for the tidal crossings .
It also summarizes the metrics that are used in the scoring which may be useful for QA/QC of data and affirming scoring methods.
...
The prioritization scores are derived from combining two primary data sources, a field-collected dataset and a remotely collected .
The geographic data are currently hosted on ArcGIS Online as a feature layer.
These point geographies contain general spatial information on each crossing (town, cross road, road classification etc.) along with the desktop assessment portion of the prioritization.
Data were migrated to this platform from the original field datasheets for increased data QA and processing effeciencies.
With that transfer desktop data found in the field data sheets are to be considered incomplete.\
Field data are currently maintained in excel spreadsheets hosted in the team's shared Box folder.
These data include all field assessement data for those sites that were visited.
This data will likely be transitioned 1to a more robust managed database structure.
These data were collected primarily under the TNC-NH/NHDES Coastal Program protocol [\^Resilient Tidal Crossings].
Deviations from these methods are documented here and will be described in more detail in other project documents.
Data extracted from Tidal Crossings Field workbooks using a suite of functions found in */functions* directory.
To save memory and time a seperate script runs the extraction process and saves an output file (.rds) to the */data* folder within this working directory.
These data can then be called in as needed here.
Alternatively, running rmdSetup.R will source all functions and run updates on data if *'dataUpdate = TRUE'*, currently the default, found near top of code chunk.

### ArcGIS app

As mentioned all the spatial and desktop assessment data are stored on ArcGIS Online.
An data [editing/entry app is here:](http://tnc.maps.arcgis.com/apps/webappviewer/index.html?id=2ec527053bc6497a9d942df70ceb9e94) for updating certain fields.
The access to the fields for edits are controlled in the underlying feature class layers in the LI Culvert Mapping group (KL and SL can access/edit etc.)

```{r data, message=FALSE, warning=FALSE, paged.print=TRUE}
LIculvertAssessments <- readRDS("data/LIculvertAssessments.rds") # tidal assessment workbooks, with varying degrees of completeness. This is the bulk of the data collected, but does not include complete desktop assessments. Those data are recorded and managed in the AGOL-hosted feature _desktopData_. 
...
LIculvertAssessments %>% 
  ...
  # Add in tidal range calculations for summary sheet.
  mutate(tidalRange_US = `HWI Stain US` - `Low Tide Water Elevation US`,
         tidalRange_DS = `HWI Stain DS` - `Low Tide Water Elevation DS`) -> heights # heights are needed for some of the calculations. Perhaps a better method would be to use SQL-like queries referencing multiple tables as needed. 
...
Scoring assigned by manually classifying crossings based on proximity to marsh complexes.
Crossings in or adjacent to large complexes (\>\~15 acres in size) scored 5, crossings in or adjacent to smaller complexes were scored a 3, and crossings disconnected from any marsh complex were scored a 1.
Manual classification was used due to inconsistent spatial relationships between the crossing locations and the marsh complexes.
Often crossings are found on the edges or outside of the boundary of the marsh complex but are in important waterways connecting these complexes.\
...
LIculvertPrioritization <- LIculvertData_location %>% rowwise() %>% 
  mutate(Score_MarshComplex = as.numeric(MarshComplex_Scoring)) %>% 
  select(crossingID, town_name, Score_MarshComplex, AssessmentStatusCode) %>%
...
The total area of salt marsh upstream of the crossing is identified using Warren Pinnacle's SLAMM *Initial Condition* layer of existing marsh habitat.
The area of saltmarsh is enumerated for each crossing as part of the desktop assessment (da_catchmentArea_saltMarsh).
The distribution of marsh areas fit a log-normal distribution so a modified log-scale was used for converting acreage into metric scores as outlined below.
...
smsu <- LIculvertData_location %>% select(crossingID, da_catchmentArea_saltMarsh) %>% rowwise() %>% 
  mutate(Score_marshAc = as.integer(cut(x = da_catchmentArea_saltMarsh, 
...
Consists of 3 **sub-metrics** that when averaged yield the Degree of Tidal Restriction and Aquatic Organism Passage (DTR/AOP) metric score. To maximize the scoring opportunities for this metric, the mean is taken from those available sub-metrics. 
The sub-metrics used and described below are:
...
The tidal range ratio is an indicator of a tidal (flow) restriction at the crossing structure.
The ratio is simply the upstream tidal range divided by the downstream tidal range as measured during the field assessments.
The tidal range is assumed to be the difference in elevation between the low tide elevation at the time of the assessment (following protocol, assessment are made at low tide) and the high water indicator stain.
A crossing structure with equal tidal ranges on both sides of the structure yields a value of 1.
A ratio of 0.5 would indicate that the downstream tidal range is twice as large as the upstream tidal range.
The tidal range is also reported in the Summary Sheet documentation for each crossing where available.

Followed NH documnetation [^1]:
![Table 7:](tidalRangeRatioTable.PNG)

[^1]: Resilient Tidal Crossings

```{r tidalrange, fig.width=6, message=FALSE, warning=FALSE, layout="l-body-outset", paged.print=TRUE}

trr <- LIculvertAssessmentData %>% rowwise() %>% 
...
The crossing ratio is a proxy for the degree in which the structure restricts the stream flow.
The crossing ratio is calculated as the ratio between the crossing structure's opening(s) and the width of the stream/tidal creek.
Both sides of the structure are considered and the greater of 2 ratios (upstream and downstream) are used to score the crossing.
**A wide river with a small structure opening will score high, indicating a restriction**.

```{r crossingratios, fig.width=6, message=FALSE, warning=FALSE, paged.print=TRUE}


cr <- LIculvertAssessmentData %>% rowwise() %>% 
  ...
  mutate(crossingRatio_upStrm = crossing_ratio(channelWidth = da_ChannelWidth_upStream,
                                               dimA = CrosDim_upA * number_structures, 
                                               dimC = CrosDim_upC * number_structures),
    crossingRatio_dwStrm = crossing_ratio(channelWidth = da_ChannelWidth_dwnStream, 
                                          dimA = CrosDim_dwnA * number_structures, 
                                          dimC = CrosDim_dwnC * number_structures),
    crossingRatio = pmax(crossingRatio_dwStrm, crossingRatio_upStrm, na.rm = TRUE),
...
Since crossing ratio is dependent on desktop assessment data (or field assessment if available), many more crossings (n = **`r LIculvertPrioritization %>% filter(!is.na(crossingRatio)) %>% nrow`**) have scores calculated.

------------------------------------------------------------------------

### E3c. Erosion Classification

The ratio of scour pool width to channel width for both up and down stream sections.
A large ratio indicates a larger scour pool.
The final Erosion Classification score is based on the *LARGER* of the 2 ratios (up and downstream)

```{r erosionClass, fig.height=5, fig.width=7, message=FALSE, warning=FALSE, paged.print=TRUE}

ec <- LIculvertAssessmentData %>% rowwise() %>% 
...
### E3. Degree of Tidal Restriction-Aquatic Organism Passage Final Scoring

This metric is calculated as the average of the 3 sub-metrics available within E3 (E3a; E3b; E3c).
This method of aggregating the scores of the tidal range ratio, crossing ratio, erosion classification ratio, was choosen to maximize the number of crossings that could be given a score provided that field data may have not been available for calculating the sub-metric scores.
These scores rely on measures of the structure made in the field data, which was not always available for field collection.
For example, the high water indicators (stain and wrack) were at times not obvious or present for field teams to measure or channel depth precluded access.
Averages were made across the available sub-metrics then rounded to the nearest whole integer value for the final Degree of Tidal Restriction-Aquatic Organism Passage score.

This set of plots are a bit of an eye-twister, but is the distribution of each of the 'submetrics' that are then color coded by their final score.
i.e.
There's 10 crossings that scored a 1 for the tidal range ratio, but a 3 for the final DOTR overall score.
There aren't any strong correlations across these submetrics (see correlation matrix below).
...
dtr <- LIculvertPrioritization %>% rowwise() %>% 
  ...
  mutate(raw_Score_dotr = degtidalrestr(tidal_rng_score = submetric_TidalRange, 
         ...
         Score_dotr = as.integer(cut(x = raw_Score_dotr, breaks = c(-Inf, 1.4999, 2.4999, 3.4999, 4.4999, Inf), labels = c(1:5), ordered_result = F))) %>% 
...
The Vegetation score metric is largely based on the NH [^2] field assessment protocols.
Functionally it is a classification of the plant community at the crossing being assessed.
The scores are derived from the presence of invasives and the species composition upstream and downstream of the crossing.
This scoring assumes that impacts to the hydrology caused by the crossing structure would result in a different species composition on either side of the crossing.
In addition crossings that connect to areas identified as *'Formerly Connected'* tidal marsh but the 1974 NYSDEC Tidal Wetlands Inventory
[^3]

[^2]: Resilient Tidal Crossings

[^3]: NYSDEC 1974 Tidal Wetlands

```{r vegetationScore, message=FALSE, warning=FALSE, paged.print=TRUE}
# 150 with scores- 185 missing.
vegsc <- LIculvertAssessmentData %>% rowwise() %>% 
  select(crossingID, Vegchoice, formerlyConnected) %>% mutate(Score_vegetation = as.integer(vegetationScore(Vegchoice, formerlyConnected)))
...
To be scored, a crossing will need to have been assessed in the field with at least most of a field survey containing mesaures of high water indicators, structure opening widths, and vegetation classification.
The Total Ecological Benefit Score is the rescaled (1 to 5) *SUM* of the 4 ecological benefit metrics.
...
# Score_dotr
# Score_vegetation
# To match the FW naming scheme- Total_XX equals the tally of the scores Score_XX = the rescaled (1-5) score
LIculvertPrioritization <- LIculvertPrioritization %>% rowwise() %>% 
  mutate(Total_EcoBenefit = sum(Score_MarshComplex, Score_marshAc, Score_dotr, Score_vegetation, na.rm = F),
    Score_EcoBenefit = as.integer(benefit_score(total.benefit.score = Total_EcoBenefit, no.metrics = 4))) %>%
  ungroup()

LIculvertPrioritization %>%  
  ggplot(aes(x = Score_EcoBenefit, fill = as.factor(Score_EcoBenefit))) +
  scale_fill_tnc("gradient1") +
  geom_bar() + 
  geom_text(stat = "count", aes(label = ..count..), position = position_stack(vjust = 0.5)) +
  labs(subtitle = "Distribution of scores", title = "Ecological Benefit Score", x = NULL ) +
  theme(legend.position = "none")
...
The probability of road inundation as estimated by the Warren Pinnacle[^4] inundation model.
This metric is a composite score which takes the 5 SLR scenarios into consideration to score wach crossing from a high score (inundated at the most conservative SLR projections) to a low score (Not within a SLR inundation area).
Each crossing was joined to each of the 5 scenarios and a series of attributes were created to identify if a crossing was within the boundary of that scenarios inundation.
...
slrIn <- LIculvertData_location %>% st_drop_geometry() %>%
  select(crossingID, starts_with("Suffolk")) %>% rowwise() %>% 
  mutate(
    ...
    Score_riskSLR_inundation = case_when(
      Suffolk_InitialCondition_Inund_Freq != 0 &
       (Suffolk_2085_LOW_Inund_Freq == 0 |
        ...
        Suffolk_2085_LOWMED_Inund_Freq == 1) ~ 5L,
      Suffolk_InitialCondition_Inund_Freq != 0 &
      (Suffolk_2085_MED_Inund_Freq == 0 |
        Suffolk_2085_MED_Inund_Freq == 1) ~ 4L,
      Suffolk_InitialCondition_Inund_Freq != 0 &
      (Suffolk_2085_HIGHMED_Inund_Freq == 0 |
        Suffolk_2085_HIGHMED_Inund_Freq == 1) ~ 3L,
      Suffolk_InitialCondition_Inund_Freq != 0 &
      (Suffolk_2085_HIGH_Inund_Freq == 0 |
        Suffolk_2085_HIGH_Inund_Freq == 1) ~ 2L,
      Suffolk_InitialCondition_Inund_Freq == 0 ~ 1L,
      TRUE ~ 1L # all others get a 1.
      # Suffolk_2085_LOW_Inund_Freq == 0 |
      #   Suffolk_2085_LOW_Inund_Freq == 1 |
      #   Suffolk_2085_LOWMED_Inund_Freq == 0 |
      #   Suffolk_2085_LOWMED_Inund_Freq == 1 ~ 5L,
...
This metric estimates the inundaion rate (how frequent is it currently being flooded) of a crossing, based on the SLAMM data under the current sea level conditions.
Crossings that were inundated frequently were given a 5, seldomly inundated crossings were given a 1.

```{r}


stormSurgeIn <- LIculvertData_location %>% st_drop_geometry() %>% rowwise() %>% 
  select(crossingID, Suffolk_InitialCondition_Inund_Freq) %>%
  mutate(
    Score_stormsuge = case_when(
      # an issue arose where crossings that were bridges were being characterized as 'open water' which 
      Suffolk_InitialCondition_Inund_Freq == 0 ~ 1L,
        Suffolk_InitialCondition_Inund_Freq == 1 ~ 5L,
      Suffolk_InitialCondition_Inund_Freq == 2 |
        Suffolk_InitialCondition_Inund_Freq == 3 ~ 4L,
      Suffolk_InitialCondition_Inund_Freq == 4 ~ 3L,
      Suffolk_InitialCondition_Inund_Freq == 5 ~ 2L,
      TRUE ~ 1L
    ),
    #   Suffolk_InitialCondition_Inund_Freq == 0 |
    #     Suffolk_InitialCondition_Inund_Freq == 1 ~ 5L,
...
rainrisk <-
  LIculvertData_location %>% select(crossingID, da_catchmentLndCover_imperv) %>% rowwise() %>%  mutate(Score_rainfallRisk = as.integer(cut(
...
Goal of this metric is to capture potential marsh migration value, assuming that the culvert does not impede that migration.
It essentially quantifies elevations within a range that would be likely to support salt marsh habitats.
Will a marsh be able to presist in this area in light of sea level rise?
Areas that have large potential marsh migration cooridors are scored high, low to no marsh migration coordidors scored low.
...
smmp <-
  LIculvertData_location %>% select(crossingID, da_MarshMigrPot_acres) %>% rowwise() %>%  mutate(Score_marshmigpotAc = as.integer(cut(
...
LIculvertPrioritization <-
  ...
  mutate(Total_Resilience =  sum(
      Score_riskSLR_inundation,
      Score_stormsuge,
      Score_rainfallRisk,
      Score_marshmigpotAc,
      na.rm = F
    ), Score_ResilienceBenefit = as.integer(benefit_score(Total_Resilience, no.metrics = 4)
  )) %>% ungroup()

LIculvertPrioritization %>%
  ggplot(aes(x = Score_ResilienceBenefit, fill = as.factor(Score_ResilienceBenefit))) +
  scale_fill_tnc("gradient1") +
  geom_bar() +
  geom_text(stat = "count",
            aes(label = ..count..),
            position = position_stack(vjust = 0.5)) +
  labs(title = "Total Resilience Benefit Score",
       subtitle = "Distribution of scores",
       x = NULL) +
  theme(legend.position = "none")
...
NYSDOT Road functional class data were spatially joined to each crossings.
Crossings not aligned with NYSDOT road data (i.e. on foot paths or small unpaved roads) were classified manually, using aerial imagery, field notes, and NYSDOT road data layers as guidance.
...
tb <-
  ...
  select(crossingID, FunctionalClass_supervised, Evac_Rte) %>% rowwise() %>% 
  mutate(
...
NYSDOT Road Evacuation routes were spatially joined to crossings, and reviewed for completeness and accuracy.
Evacuation routes as defined in this metric are officially designated routes and are typically major thoroughfares.
The scoring for this metric was simple a binary choice, crossings along designated evacuation routes were scored a 5; all others were given a 1.
...
LIculvertPrioritization <- LIculvertPrioritization %>% rowwise() %>% 
    mutate(Total_TranBenefit = sum(Score_Rd_EvacRte, Score_Rd_Funclass, na.rm = F),
           Score_TranBenefit = benefit_score(Total_TranBenefit, no.metrics = 2)) %>% ungroup()

tbs <- LIculvertPrioritization %>% rowwise() %>% 
  select(crossingID, Score_Rd_EvacRte, Score_Rd_Funclass, Total_TranBenefit, Score_TranBenefit)

LIculvertPrioritization %>%  
  ggplot(aes(x = Score_TranBenefit, fill = as.factor(Score_TranBenefit))) +
  scale_fill_tnc("gradient1") +
  geom_bar() + 
  geom_text(stat = "count", aes(label = ..count..), position = position_stack(vjust = 0.5)) +
...
*Current crossing condition* evaluates several components of the crossing structure to effectively summarize the overall condition.
Structure condition, such as headwall and wingwall condition.
Scour has been removed as that condition is found in *Eroision Classification*.

```{r crossingConditionEval, message=FALSE, warning=FALSE, paged.print=TRUE}
# 149 scored- 186 = NA

crscond <- LIculvertAssessmentData %>% st_drop_geometry() %>% rowwise() %>% 
  select(
    crossingID,
...
# All complete.
partpr <- LIculvertAssessmentData %>% rowwise() %>% 
...
For a crossing to be scored there needs to be either a cross sectional profile surveyed or the crossing needs to be identified as submerged at low tide.
Essentially the crossing would need to be visited and some level of assessment would be needed.
...
# lack of clearance = ceiling ht - HWI stain, Indicates the regular level of inundation to the crossing structure
...
highwater <- LIculvertAssessmentData %>% rowwise() %>% 
  select(
    crossingID, 
    AssessmentStatusCode, # if assessed as submerged then score a 5. 
    starts_with("HWI"),
    starts_with("Road "),
  ...
  mutate(
    submerged = if_else(AssessmentStatusCode == 7, true = 5, false = NA_real_), # if submerged give a value of 5 and pass this value to the final score function
    ...
    score_highwater_dif_US = as.numeric(
      as.character(cut(x = highwater_dif_US, breaks = c(-Inf, 0, 1.5, 3, 6, Inf),
      labels = 5:1,
    ...
    score_highwater_dif_DS = as.numeric(
      as.character(cut(x = highwater_dif_DS, breaks = c(-Inf, 0, 1.5, 3, 6, Inf),
      labels = 5:1,
      ordered_result = FALSE))
    ),
    ...
    score_clearance_dif_DS = as.numeric(
      as.character(cut(x = clearance_dif_DS, breaks = c(-Inf, 0, 1, 2, 3, Inf),
      labels = 5:1,
      ordered_result = FALSE))
    ),
    ...
    score_clearance_dif_US = as.numeric(
      as.character(cut(x = clearance_dif_US, breaks = c(-Inf, 0, 1, 2, 3, Inf),
      labels = 5:1,
      ordered_result = FALSE))
    ...
    Score_highWaterClearance = pmax(na.rm = T,
      ...
      submerged # this will be 5 if classified as submerged otherwise a 1. 
    ))
...
The ratio of scour pool width to channel width for both up and down stream sections.
A large ratio indicates a larger scour pool.
Score is based on the larger of the 2 ratios.
This metric aims to identify crossings that have potential erosion issues that are impacting the condition of the structure.
This ratio is reliant on data collected using the desktop assessments protocol.
...
ec2 <- ec %>% rowwise() %>% mutate(Score_erosionconditionClass = submetric_erosionClass) %>% select(-submetric_erosionClass)
...
*NOTE:* Does not include the High water risk/Lack of clearance metrics for *ALL* crossings.

#### SPECIAL NOTE:

After discovering coding bug, this submetric was reran to include 2 scoring methodologies to determine the change that these methods have in the final scoring, as well as within the Infrastructure Condition Benefit Score.
version a (\*\*\_va) is taking the approach of using as much data as possible for the high-water risk/lack of clearance sub-metric score.
That means that when a field assessment was performed and sufficient HWI (both stain or wrack) measurements were collected a *risk of highwater/lack of clearance* submetric score was calculated and included in the Total Infrastructure Benefit score.
**IF** there was a lack of data, then the *Total Infrastructure Benefit* score was calculated based on only the other 3 available metrics (C1, C2, C4).

Version B (\*\*\_vb) drops the *risk of highwater/lack of clearance* submetric score for all crossings regardless of their being suffecient data.
...
LIculvertPrioritization <-
  LIculvertPrioritization %>% rowwise() %>%
  mutate(Total_InfraConditionBenefit = sum(
        Score_CrossConditon,
        Score_partPrior,
        Score_highWaterClearance,
        # NOTE: Dropped C3 from Total scoring as a test to see how we might be able to bump our total # scored up.
        Score_erosionconditionClass,
        na.rm = F # na.rm is FALSE so that crossings with missing scores will be dropped
      ),
    Score_InfrastructureBen = case_when(
    is.na(Score_highWaterClearance) ~ benefit_score(
      total.benefit.score = sum(
        Score_CrossConditon,
        Score_partPrior,
        # Score_highWaterClearance, # NOTE: Dropped C3 from Total scoring as a test to see how we might be able to bump our total # scored up.
        Score_erosionconditionClass,
        na.rm = F # na.rm is FALSE so that crossings with missing scores will be dropped
      ),
      no.metrics = 3
...
LIculvertPrioritization %>%  
  ggplot(aes(x = Score_InfrastructureBen, fill = as.factor(Score_InfrastructureBen))) +
  scale_fill_tnc("gradient1") +
  geom_bar() + 
  geom_text(stat = "count", aes(label = ..count..), position = position_stack(vjust = 0.5)) +
...
LIculvertPrioritization <- LIculvertPrioritization %>% rowwise() %>%
  mutate(
    Total_Benefit = as.integer(sum(  
      Score_EcoBenefit,
      Score_TranBenefit,
      Score_ResilienceBenefit,
      Score_InfrastructureBen,
      na.rm = F
    ))
  ) %>% ungroup()

LIculvertPrioritization %>%  
  ggplot(aes(x = Total_Benefit, fill = as.factor(Total_Benefit))) +
  scale_fill_tnc("gradient1") +
  geom_bar() + 
  geom_text(stat = "count", aes(label = ..count..), position = position_stack(vjust = 0.5)) +
...
## Our team went through all the scored crossings, both freshwater and tidal protocols and
## decided on a list of the top 10 crossings that would provide the most benefit from an upgrade.
## These are flagged by creating a field that highlights those crossings. 
top10 <- read_rds("top10.rds")

LIculvertPrioritization <- crossingLocations %>% 
  left_join(LIculvertPrioritization) %>% 
  mutate(priorityCrossing = crossingID %in% top10) 
...
As the map shows there are quite a few crossings that have been marked as complete that lack the data needed for a score.
QA may fix some of these.

```{r totalScoreMap, echo=FALSE, fig.height=5, fig.width=10, message=FALSE, warning=FALSE}
# tmap::tmap_mode("plot")
tmap_options(limits = c(facets.view = 7), basemaps = "Esri.WorldImagery")
tmap::tmap_mode("view")
LIculvertPrioritization %>% 
  left_join({LIculvertAssessmentData %>% st_drop_geometry()}) %>% 
  tm_shape() + 
  tm_text(text = "Total_Benefit", size = 1) +
  # tm_facets(by = "Rd_Owner") + 
  tm_dots(title = "Total Prioritization Score", 
  size = .1,
  col = "Total_Benefit",
  colorNA = "grey40",
  palette = tnc_pal("gradient1")(5),
  popup.vars = c("Crossing ID:" = "crossingID",
                 "Ecological Benefit:" = "Score_EcoBenefit",
                 "Resilience Benefit:" = "Score_ResilienceBenefit",
                 "Transportation Benefit" = "Score_TranBenefit",
                 "Infrastructure Benefit" = "Score_InfrastructureBen",
                 "Total Prioritization Score" = "Total_Benefit"
                 )
)
...
LIculvertPrioritization %>% st_drop_geometry() %>%
  select(Score_MarshComplex, Score_marshAc, Score_dotr, Score_vegetation, Total_EcoBenefit) %>% 
  ggcorr(
    hjust = 1,
    size = 4,
    layout.exp = 1,
...
LIculvertPrioritization %>% st_drop_geometry() %>%
  select(Score_Rd_EvacRte, Score_Rd_Funclass, Total_TranBenefit) %>% 
  ggcorr(
    hjust = 1,
    size = 4,
    layout.exp = 1,
...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
LIculvertPrioritization %>% st_drop_geometry() %>%
  select(Score_CrossConditon, Score_partPrior, Score_erosionconditionClass) %>% 
  ggcorr(
    hjust = 1,
    size = 4,
    layout.exp = 1,
...
LIculvertPrioritization %>% st_drop_geometry() %>%
  select(starts_with("Score"), starts_with("sub"), starts_with("Total")) %>%
  ggcorr(angle = 0,
    hjust = .9,
    size = 4,
    layout.exp = 1,
...
We might want to think about how our final scoring rolls up into the *Total_Prioritization* score as there are a quite a high number of missing scores/data that are limiting the total number of scores generated.
Of the `r nrow(LIculvertAssessmentData)` tidal culverts identified, `r sum(!is.na(LIculvertAssessmentData$dateAssessed))` were assessed and of those only `r sum(!is.na(LIculvertPrioritization$Total_Prioritization))` were complete enough to be fully scored.
*Total Infrastructure Benefit* scores are likely the most limiting, as things are currently calculated.
Both of these scores rely on the

### QA Total Prioritization

```{r fig.height=11, fig.width=11, message=FALSE, warning=FALSE, paged.print=FALSE}
LIculvertPrioritization %>% st_drop_geometry() %>% arrange(Total_Benefit, Score_EcoBenefit, Score_InfrastructureBen, Score_TranBenefit, Score_ResilienceBenefit) %>% 
...
LIculvertPrioritization %>% st_drop_geometry() %>% 
  ...
  Score_vegetation,
  Score_EcoBenefit
) %>% remove_rownames() %>%  arrange(Score_EcoBenefit) %>% 
  mutate_all(.funs = as.numeric) %>%
  column_to_rownames(var = 'crossingID') %>% 
    d3heatmap(
      na.rm = T,
...
LIculvertPrioritization %>% st_drop_geometry() %>% 
  select(crossingID,
  Score_Rd_EvacRte, Score_Rd_Funclass, Total_TranBenefit
) %>% remove_rownames() %>%  
  mutate_all(.funs = as.numeric) %>%
  column_to_rownames(var = 'crossingID') %>% 
    d3heatmap(
...
LIculvertPrioritization %>% 
  select(crossingID, Total_TranBenefit, ss_Functional_Class) %>% st_drop_geometry() %>% 
  replace_na(replace = list(Total_TranBenefit = 0)) %>%
  ggplot(aes(x = ss_Functional_Class, y = Total_TranBenefit)) + 
  geom_jitter() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

```

Unscored crossings are all RR crossings that weren't given a 'functional road class', because they aren't roads.
THoughts?
Should we give a score for these?
...
LIculvertPrioritization %>% st_drop_geometry() %>% 
  select(crossingID,
  Score_CrossConditon, Score_erosionconditionClass, Score_partPrior, Score_highWaterClearance, Score_InfrastructureBen
) %>% remove_rownames() %>%  arrange(Score_InfrastructureBen, Score_highWaterClearance, Score_CrossConditon, Score_erosionconditionClass, Score_partPrior) %>% 
  mutate_all(.funs = as.numeric) %>%
  column_to_rownames(var = 'crossingID') %>% 
    d3heatmap(
      na.rm = T,
...
qa_data <- highwater %>% st_drop_geometry() %>% 
  mutate(checkRoadHt_HWI = if_else(`Road Surface US` < `HWI Stain US` | `Road Surface DS` < `HWI Stain DS`, true = "Yes", "NO")) %>% 
  select(crossingID, checkRoadHt_HWI) %>% left_join(qa_data)
...
all_vars_heat <- LIculvertPrioritization %>%
  st_drop_geometry() %>% 
  select(crossingID, Score_MarshComplex, Score_marshAc, Score_dotr, Score_vegetation, Score_EcoBenefit, Score_riskSLR_inundation, Score_stormsuge, Score_rainfallRisk, Score_marshmigpotAc, Score_ResilienceBenefit, Score_Rd_EvacRte, Score_Rd_Funclass, Score_TranBenefit, Score_CrossConditon, Score_partPrior, Score_highWaterClearance, Score_erosionconditionClass, Score_InfrastructureBen, Total_Benefit) %>% 
   mutate_all(.funs = as.numeric) %>% 
  add_n_miss() %>% 
  dplyr::arrange(Total_Benefit) %>% 
   column_to_rownames(var = 'crossingID')
...
As this project matured there was a need to develop a more stringent data management system for managing data and provide QA controls as well as ease the transition into map products.
A relational database was constructed for this purpose and was largely modelled after the NH SADES database (link).
At the time of writing the prioritization scoring methods are all conducted within R.
For the sake of reproducibility the prioritization scoring will remain in the R environment and will be scripted in a way to update/overwrite the proritization data as needed (following data additions or new methodology).
As time allows these prioritizations can be migrated into ArcGIS tool kits or other scripted calculators (Arcade language perhaps)

```{r writeDB, message=FALSE, warning=FALSE, include=FALSE}
# source("utilities/tidalCrossings_DatabaseBuiler.R")
```
...
[^6] <http://gis.ny.gov/gisdata/metadata/nysdec.tidal_wetlands_lower.html>

[^6]: NYSDEC 1974 Tidal Wetlands

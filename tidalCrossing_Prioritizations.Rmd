...
This metric is calculated as the average of the 3 sub-metrics available within E3 (E3a; E3b; E3c). This method of aggregating the scores of the tidal range ratio, crossing ratio, erosion classification ratio, was choosen to maximize the number of crossings that could be given a score provided that field data may have not been available for calculating the sub-metric scores. A number of these scores rely on data that was not always available for field collection. For example, the high water indicators (stain and wrack) were at times not obvious or present for field teams to assess. Averages were made across the sub-metrics that were available then rounded to the nearest whole integer value for the final score.
...
LIculvertPrioritization <- LIculvertPrioritization %>% rowwise() %>% 
  mutate(Total_Eco = as.integer(benefit_score(total.benefit.score = sum(Score_MarshComplex, Score_marshAc, Score_dotr, Score_vegetation, na.rm = F), no.metrics = 4))) %>%
  ungroup()
...
rainrisk <-
  LIculvertData_location %>% select(crossingID, da_catchmentLndCover_imperv) %>% mutate(Score_rainfallRisk = as.integer(cut(
...
smmp <-
  LIculvertData_location %>% select(crossingID, da_MarshMigrPot_acres) %>% mutate(Score_marshmigpotAc = as.integer(cut(
...
LIculvertPrioritization <-
  ...
  mutate(Total_Resilience = as.integer(benefit_score(
    sum(
...
LIculvertPrioritization <- LIculvertPrioritization %>% rowwise() %>% 
    mutate(Total_Trans = benefit_score(sum(Score_Rd_EvacRte, Score_Rd_Funclass, na.rm = F), no.metrics = 2)) %>% ungroup()
...
For a crossing to be scored there needs to be either a cross sectional profile surveyed or the crossing needs to be identified as submerged at low tide. Essentially the crossing would need to be visited and some level of assessment would be needed.
...
The ratio of scour pool width to channel width for both up and down stream sections. A large ratio indicates a larger scour pool. Score is based on the larger of the 2 ratios. This metric aims to identify crossings that have potential erosion issues that are impacting the condition of the structure. This ratio is reliant on data collected using the desktop assessments protocol.
...
version a (**_va) is taking the approach of using as much data as possible for the high-water risk/lack of clearance sub-metric score. That means that when a field assessment was performed and sufficient HWI (both stain or wrack) measurements were collected a _risk of highwater/lack of clearance_ submetric score was calculated and included in the Total Infrastructure Benefit score. __IF__ there was a lack of data, then the _Total Infrastructure Benefit_ score was calculated based on only the other 3 available metrics (C1, C2, C4). 

Version B (**_vb) drops the  _risk of highwater/lack of clearance_ submetric score for all crossings regardless of their being suffecient data. This is a simpler approach. 
...
LIculvertPrioritization <-
  LIculvertPrioritization %>% rowwise() %>%
  mutate(Total_InfrastructureBen_va = case_when(
    ...
    !is.na(Score_highWaterClearance) ~ benefit_score( # when there is a 
        total.benefit.score = sum(
          Score_crossConditon,
          Score_partPrior,
          Score_highWaterClearance, # NOTE: Dropped C3 from Total scoring as a test to see how we might be able to bump our total # scored up.
  ...
  Total_InfrastructureBen_vb = benefit_score(
        total.benefit.score = sum(
          Score_crossConditon,
          Score_partPrior,
          # Score_highWaterClearance, # NOTE: Dropped C3 from Total scoring as a test to see how we might be able to bump our total # scored up.
  ...
  Total_InfrastructureBen = Total_InfrastructureBen_va # To allow for continued processing while descisions are made migrate the preferred scoring method to the original variable name.
  ) %>% ungroup()
...
LIculvertPrioritization <- LIculvertPrioritization %>% rowwise() %>%
  mutate(
    Total_Prioritization_va = as.integer(sum( ## TODO: Remove this split once decision is made.
      Total_Eco,
      Total_InfrastructureBen_va,
      Total_Resilience,
      Total_Trans,
      na.rm = F
    )), 
    Total_Prioritization_vb = as.integer(sum( ## TODO: Remove this split once decision is made.
      Total_Eco,
      Total_InfrastructureBen_vb,
      Total_Resilience,
      Total_Trans,
      na.rm = F)),
    Total_Prioritization = Total_Prioritization_va # TODO: Remove after diagnosis. Same as the total infr bene score above.
  ) %>% ungroup()
...
LIculvertPrioritization %>% 
  ...
  # filter(Total_Prioritization == 16) %>%
# LIculvertPrioritization %>% 
  # filter(AssessmentStatus == "Complete") %>%
  # tm_basemap("Esri.WorldImagery") +
  tm_shape() + 
  tm_text(text = "Total_Prioritization", size = 1) +

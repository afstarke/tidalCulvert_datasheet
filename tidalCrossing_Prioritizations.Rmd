...
smsu %>% 
  ggplot(aes(x = da_saltMarshArea, fill = as.factor(Score_marshAc))) + 
...
vegsc %>% select(Score_vegetation) %>% 
  mutate(Score_vegetation_na = replace_na(Score_vegetation, replace = -2)) %>% 
  ggplot(aes(x = Score_vegetation_na)) + 
  geom_histogram() + 
...
LIculvertPrioritization <- LIculvertPrioritization %>% rowwise() %>% 
  mutate(Total_Eco = benefit_score(sum(Score_MarshComplex, Score_marshAc, Score_dotr, Score_vegetation, na.rm = T))) %>% ungroup()
...
#TODO: Update AGOL feature layer to be current with catchment data. 

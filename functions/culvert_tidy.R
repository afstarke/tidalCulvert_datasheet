...
#' @examples
#'
crossSection <- function(filepath, tidycells, lidarHts) {
  # Set up variables for adjusting to NAVD88 with surveyHtCorrection()
  ...
  Longprofile <-
    channelLongidinalProfile_extract(filepath, tidycells, lidarHts)
  USinvert <-
    Longprofile %>% filter(str_detect(`Feature Code`, pattern = "I")) %>%
